...
def register_procrastination(current_user):
    ...
    db.session.execute(db.update(User).where(User.email == current_user.email).values(last_submission=datetime.datetime.now()))
    db.session.commit()
    # create a new event
    new_event = Event(type="procrastination",event_date=datetime.datetime.strptime(req["event_date"],"%Y-%m-%d"),data={"test_key": "test_value"},owner=current_user.id)
